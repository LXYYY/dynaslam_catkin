...
class MaskRCNN():
    ...
    def save_as_tf_pb(self):
        # doesn't really work, cannot be loaded somehow
        sess = K.get_session()
        ...
        tf.train.write_graph(frozen_graph, ".", "mask_rcnn.pb", as_text=False)
        
#         name='mask_rcnn_pb.pb'
#         outdir='./src/python'
#         numoutputs=7    
#         pred=[None]*numoutputs
#         pred_node_names=[None]*numoutputs
#         self.output_names=['detections', 'mrcnn_class', 'mrcnn_bbox', 'mrcnn_mask', 'rpn_rois', 'rpn_class', 'rpn_bbox']
#         pred_node_names=["output_"+name for name in self.output_names]
#         self.numoutputs=self.output_names.__len__()
#         pred=[tf.identity(self.keras_model.output[i],name=pred_node_names[i]) for i in range(numoutputs)]
